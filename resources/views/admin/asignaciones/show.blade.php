...
@section('content')
    ...
    <div class="card">
                    ...
                    <div class="row">
                        ...
                        @foreach ($logros as $logro)
                            ...
                            <div class="card" style="width: 18rem;">
                                <img class="card-img-top" src="{{Storage::url($logro->image->url)}}" alt="Card image cap">
                                <div class="card-body">
                                    <h5 class="card-title">{{$logro->nombre}}</h5>
                                    <p class="card-text">{{$logro->descripcion}}</p>

                                <form class="text-center" action="{{route('admin.asignaciones.destroy', $alumno->user_id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="card-footer">
            <a  class="btn btn-success" href="{{ route('admin.asignaciones.edit', $alumno->user_id."-".$tarea_id)}}"> Asignar un nuevo Logro</a>
        </div>
    </div>
@stop
